...
switch direction {
	case 0: 
		if(!right) {x+= tile_width; future_x = x + tile_width} break;
	case 90: 
		if (!top) {y-= tile_height; future_y = y - tile_height} break;
	case 180: 
		if (!left) {x-= tile_width; future_x = x - tile_width} break;
	case 270: 
		if (!down) {y+= tile_height; future_y = y + tile_height} break;
}
